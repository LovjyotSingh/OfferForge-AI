...
async function callGemini(prompt, options = {}) {
  ...
  const fallbackModels = [requestedModel, 'gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-2.0-flash-exp'].filter((v, i, a) => a.indexOf(v) === i);
  ...
  for (const model of fallbackModels) {
    ...
    try {
      ...
      // If quota/model error, try next fallback model
      continue;
    }
  }
